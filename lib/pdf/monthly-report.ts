...
export function generateMonthlyReportPdf(data: DailyReportData) {
    const doc = new jsPDF({
    ...
    doc.setFontSize(16)
    doc.text("Monthly Expense Report", 14, 18)

    doc.setFontSize(10)
    doc.setTextColor(100)
    doc.text(`User: ${data.user}`, 14, 26)
    doc.text(`Month: ${data.monthName}`, 14, 32)
    ...
    doc.save(`report-of-${data.monthName}.pdf`)
}
